...
\noindent Revenons au problème. Soit $x\in H\setminus C$ fixé. Posons $x^*=p_C(x)$, $e=\frac{x-x^*}{\|x-x^*\|}$ et $H$ l'hyperplan d'appui en $x^*$ de normale $e$, de sorte que $$H=\{y\in H, \langle y,e\rangle = \langle x^*,e\rangle\}$$
\textbf{Lemme}: On note $p_H$ la projection orthogonale sur $H$ (qui est un sev fermé). Soit $y\in H^+$ et $z\in H^-$. Alors $\|y-p_H(y)\|\leq \|y-y'\|$.  \newline \newline
\underline{Preuve}: C'est géométriquement évident (faire un dessin). Formellement, comme 
$\begin{aligned}[t]\|y-y'\|^2&=\|y-p_H(y) + p_H(y)-y'\|^2 \\
&= \|y-p_H(y)\|^2 + \|p_H(y)-y'\|^2 -2\langle y-p_H(y), y'-p_H(y)\rangle
\end{aligned}$\newline
il suffit de prouver que $\langle y-p_H(y), y'-p_H(y)\rangle \leq 0$ (ce qui est encore une évidence géométrique).\newline
Comme $y-p_H(y)\in H^\bot$ (cf théorème de la projection orthogonale sur un sev fermé), et que $H^{\bot}=\operatorname{Vect}(e)$ on peut écrire \newline
$y-p_H(y) = \lambda e$ d'où $\begin{aligned}[t]\langle y-p_H(y), e\rangle &= \lambda \|e\|^2=\lambda\\ 
&= \langle y, e\rangle - \langle p_H(y), e\rangle \\
&\geq \langle x^*, e\rangle - \langle p_H(y), e\rangle \quad \text{car }y\in H^+ \\
&= \langle x^*, e\rangle - \langle x^*, e\rangle \quad \text{car }p_H(y)\in H \\
&= 0 \end{aligned} $\newline 
donc $\lambda \geq 0$.\newline
Finalement, $\begin{aligned}[t] \langle y-p_H(y), y'-p_H(y)\rangle &= \lambda \langle e, y'-p_H(y) \rangle \\
&= \lambda \left( \langle e, y'\rangle - \langle e, p_H(y) \rangle \right)\\
&\leq \lambda \left( \langle e, x^*\rangle - \langle e, x^* \rangle \right) \; \text{car } \lambda\geq 0,\; z\in H^-, \;p_H(y)\in H \\
&= 0
\end{aligned}$\newline
comme souhaité. \null \hfill $\square$\\

\noindent Dans notre problème on dispose d'un voisinage de $x$ noté $U$ tel que $U\subset H^+$. Pour $y\in U$, comme $p_C(y)\in C\subset H^-$, le lemme donne $(d(y,H))^2\leq (d(y,C))^2$. On a aussi la majoration triviale $(d(y,C))^2\leq \|y-x^*\|^2$. En bref $$(d(y,H))^2\leq (d(y,C))^2\leq \|y-x^*\|^2$$
Par ailleurs $(d(y,H))^2$ se réécrit simplement $$\langle y-p_H(y),e\rangle^2  = \langle y-x^*,e\rangle^2 = \langle y-x^*,\frac{x-x^*}{\|x-x^*\|}\rangle^2 $$
