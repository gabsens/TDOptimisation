...
\\ 
\\
\noindent 1. Sans hypothèse de concavité en $\theta$ sur $L$, tout ce qu'on peut affirmer est que si $\theta$ est solution du système de l'énoncé, alors $\hat \theta$ est un maximum local de $\log L$, donc un maximum local de $L$.\newline
\newline
2. Etant donné $x\in \mathbb R^n$, on cherche $\displaystyle \max_{\delta >0, m \in \mathbb R} L(x,m,\delta)=\frac{1}{(2\pi)^{n/2}\delta}e^{-\frac{\sum_{i=1}^n (x_i-m)^2}{2\delta}}$.\newline
